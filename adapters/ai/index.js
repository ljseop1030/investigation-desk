...
//
// 전선에 나가는 것은 characterId, history, playerName 셋이다. style도
// bubbles도 보내지 않는다. 서버가 characterId로 자기 콘텐츠에서 꺼낸다.
// 클라이언트가 말해주는 것은 무엇도 신뢰하지 않는다는 성질이 계약 모양
// 하나로 지켜진다.
//
// playerName만은 예외다. 게임 상태가 아니라 플레이어가 시작 화면에서 방금
// 입력한 자기 데이터이고, P10에서 계정이 생기기 전까지 서버가 알 방법이 없다.
// 이 예외가 사라지는 시점이 P10이다.
...
export function createAi({ fetch, endpoint = ENDPOINT } = {}) {
  return {
    ...
    async reply(characterId, log, playerName = '') {
      try {
        const res = await fetch(endpoint, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ characterId, history: toHistory(log), playerName }),
        });
