...
test('연도로 시작하는 날짜 줄은 건드리지 않는다', () => {
  // `2024. `을 목록 머리표로 읽으면 연도가 사라진다. f1.a1의 keys에 2024가
  // 있어, 지워진 말풍선을 보고 옮겨 적으면 반려당한다.
  assert.equal(strip('2024. 3. 9. 02:41경 사고지점 통과'), '2024. 3. 9. 02:41경 사고지점 통과');
});

test('월-일로 시작하는 날짜 줄은 건드리지 않는다', () => {
  // 머리표 숫자에 상한(\d{1,2})만 두면 이쪽이 남는다. 월은 언제나 한두 자리다.
  assert.equal(strip('3. 13. 17:10 검거'), '3. 13. 17:10 검거');
});
